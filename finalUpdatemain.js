...
function addFood() {
    ...
    const foodTag = document.getElementById("foodTag").value;

    //make sure only alphabet letters
    if (!foodName.match(/^[A-Za-z]+$/) || !foodID.match(/^[A-Za-z-]+$/) || !foodTag.match(/^[A-Za-z]+$/)) {
        alert("please only alphabetic characters !");
        return;
    }
    
    //tag must already exist - doesnt work
    if (!foodTagsCheckboxes.includes(foodTag)) {
        alert("Tag does not exist ;(");
        return;
    }
    
    //make sure none of the boxes are empty
    if (foodName && foodID && foodTag) {
        //create newFood object
        const newFood = {name: foodName, id: foodID, tag: [foodTag.toLowerCase()]};
        ...
        document.getElementById("foodTag").value = "";
        ...
        alert("please enter all fields: name, id and tag !");
    }
}

//function modifyFood() { //form works but doesnt when in modal
...
//function openModifyModal() { doesnt work
//    const modifyModal = document.querySelector('.modifymodal');
//    modifyModal.showModal();
//}
//
//function initializeModal4() { doesnt work 
//    const modifyingOpenButtons = document.querySelectorAll('.ri-pencil-fill');
//    const modifyModals = document.querySelectorAll('.modifymodal');
//    
//    modifyingOpenButtons.forEach(button => {
//        button.addEventListener('click', () => {
//           index = Array.from(modifyingOpenButtons).indexOf(button);
